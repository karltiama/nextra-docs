...
In practice, { ...note } creates a new object with copies of all the properties from the note object. When we add properties inside the curly braces after the spread object
e.g. { ...note, important: true }, then the value of the important property of the new object will be true. In our example
...
More importantly make sure we understand what map is doing here for us
...
Cleaning up the App component. In practice we should always follow single responsibility principle.
...
The module returns and object that has three functions (getAll, create, and update) as its properties that deals with notes
